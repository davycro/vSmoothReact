...
class VideoProcessor {
  ...
  async build(options) {
    ...
    const vidstabdetectArgs = ['-i', inFile, '-vf', `vidstabdetect=shakiness=${shakiness}:accuracy=${accuracy}:tripod=${tripod}:result=${transformFile}`, vidstabdetectFile];
    const vidstabtransformArgs = ['-i', vidstabdetectFile, '-vf', `vidstabtransform=input=${transformFile}:optalgo=${optalgo}:smoothing=${smoothing}:crop=${crop}:maxangle=${maxangle}:maxshift=${maxshift}:tripod=${tripod}`, outFile];
  ...
  _run(args, options) {
    ...
    return new Promise((resolve, reject) => {
      const ffmpeg = spawn(binFile, args); // logging parameters
